...
input_directory = 'gs://dataproc-96b8f908-f706-4622-a182-092011579da3-us-west2/hadoop/tmp/bigquery/pyspark_input'.format(bucket)
...
# Display 10 results.
pprint.pprint(name_counts.take(10))

# Stage data formatted as newline-delimited JSON in Google Cloud Storage.
output_directory = 'gs://dataproc-96b8f908-f706-4622-a182-092011579da3-us-west2/hadoop/tmp/bigquery/pyspark_output'.format(bucket)
partitions = range(name_counts.getNumPartitions())
output_files = [output_directory + '/part-{:05}'.format(i) for i in partitions]

(name_counts
 .map(lambda (w, c): json.dumps({'name': w, 'number': c}))
 .saveAsTextFile(output_directory))

# Shell out to bq CLI to perform BigQuery import.
subprocess.check_call(
    'bq load --source_format NEWLINE_DELIMITED_JSON '
    '--schema name:STRING,number:INTEGER '
